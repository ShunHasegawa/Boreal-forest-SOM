...
create_trans_boxplot <- function(x, data, lambda = seq(-5, 5, .01), ...){ # x = formula
  
  # if(require(MASS)) install.packages("MASS")
  
  # get box-cox value
  a <- boxcox(x, data = data, plotit = FALSE, lambda = lambda, ...)
...
get_star <- function(pval, dagger = TRUE){
  ...
  cut(pval, right = FALSE, include.lowest = TRUE, 
